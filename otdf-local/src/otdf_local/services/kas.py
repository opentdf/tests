...
class KASService(Service):
    ...
    def start(self) -> bool:
        ...
        # The env var wins over logger.level in the config file, so km3 has to be set
        # here too or its debug cache lines never reach the log the audit fixture reads.
        log_level = "debug" if self.is_kao_uri else "info"
        ...
        self._process = self._process_manager.start(
            ...
            log_file=log_file,
            env={"OPENTDF_LOG_LEVEL": log_level},
        )
