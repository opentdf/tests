...
def central_directory(path: Path) -> list[CentralDirectoryEntry]:
    ...
    for _ in range(entry_count):
        ...
        # the extra field. If it doesn't -- missing entirely, or truncated --
        # that is the malformed-container case _parse_zip64_extra's docstring
        # describes, not a legitimate "no ZIP64 here" reading. Falling back to
        # the raw sentinel (0xFFFFFFFF) below would mislabel it as an ordinary
        # in-window value instead.
        if want_uncompressed and z_uncompressed is None:
            raise MalformedZipError(
                f"entry {name!r}: uncompressed size holds the ZIP64 sentinel "
                "but its extra field is missing or truncated"
            )
        if want_compressed and z_compressed is None:
            raise MalformedZipError(
                f"entry {name!r}: compressed size holds the ZIP64 sentinel "
                "but its extra field is missing or truncated"
            )
        if want_offset and z_offset is None:
            raise MalformedZipError(
...
def assert_zip64_above_4gib(entries: list[CentralDirectoryEntry]) -> None:
    ...
    for e in entries:
        ...
        if e.uncompressed_size >= ZIP64_WINDOW_HIGH:
            assert e.raw_uncompressed_size == ZIP64_SENTINEL_32 and e.has_zip64_extra, (
        ...
        if e.compressed_size >= ZIP64_WINDOW_HIGH:
            assert e.raw_compressed_size == ZIP64_SENTINEL_32 and e.has_zip64_extra, (
                f"entry {e.name!r} is {e.compressed_size} bytes compressed, at "
                f"or above 2**32, but its 32-bit compressed-size field holds "
                f"{e.raw_compressed_size} rather than the ZIP64 sentinel\n"
                + describe(entries)
            )
