...
# Build the script
monolog DEBUG "Building cli class file [${JAVA_CLI_DIR}/nanotdf3.java]"
if ! javac -cp ":${JAVA_JAR_DIR}/*" "${JAVA_CLI_DIR}/nanotdf3.java"; then
  monolog ERROR "Failed to compile [${JAVA_CLI_DIR}/nanotdf3.java]"
  exit 1
fi
...
java -cp "${JAVA_CLI_DIR}:${JAVA_JAR_DIR}/*" nanotdf3 "$STAGE" "$SOURCE" "$TARGET" "$OWNER" "$ACTION" "${@}"
