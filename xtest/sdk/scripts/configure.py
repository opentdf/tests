#!/usr/bin/env python3
"""Configure SDK artifacts for xtest.

Supports installing released artifacts from registries (fast, deterministic)
or delegating to source checkout+build for head/branch testing.

Usage:
    # Install latest stable releases for all SDKs
    python3 configure.py stable

    # Install LTS versions for all SDKs
    python3 configure.py lts

    # Install specific released versions
    python3 configure.py release go:v0.24.0 js:0.4.0 java:v0.9.0

    # Source build (delegates to checkout-all.sh / checkout-sdk-branch.sh + make)
    python3 configure.py tip
    python3 configure.py tip go
...
list_java_github_releases = _lv.list_java_github_releases
parse_semver = _lv.parse_semver
is_stable = _lv.is_stable
semver_sort_key = _lv.semver_sort_key

# LTS versions from resolve-version.py
_rv = import_module("resolve-version")
lts_versions: dict[str, str] = _rv.lts_versions
...
def main() -> None:
    ...
    p_stable.add_argument(
        ...
        nargs="*",
        default=["go", "js", "java"],
        help="SDKs to install (default: all)",
    )
    ...
    p_tip.add_argument(
        ...
        nargs="*",
        default=["go", "js", "java"],
        help="SDKs to build (default: all)",
    )
