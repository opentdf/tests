...
def resolve(sdk: str, version: str, infix: None | str) -> ResolveResult:
    ...
    try:
        ...
        remote_tags = [r.split('\t') for r in repo.ls_remote(sdk_url, tags=True).split('\n')]
        all_listed_tags = [(sha, tag.split('refs/tags/')[-1]) for (sha, tag) in remote_tags if 'refs/tags/' in tag]
        listed_tags = all_listed_tags
        if infix:
            listed_tags = [(sha, tag.split(f'{infix}/')[-1]) for (sha, tag) in listed_tags if f'{infix}/' in tag]
        ...
        if version == "latest":
            ...
            matching_tags = [(sha, tag) for (sha, tag) in listed_tags if tag in [version, f"v{version}"]]
            if not matching_tags:
                raise ValueError(f"Tag [{version}] not found in [{sdk_url}]")
            sha, tag = matching_tags[0]
            return {"sdk": sdk, "tag": tag, "sha": sha}
    except Exception as e:
        return {"sdk_name": sdk, "err": f"Error resolving version {version} for {sdk}: {e}"}
