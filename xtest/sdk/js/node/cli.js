...
const clientFor = program => {
  ...
  console.log(`Constructing Client: ${JSON.stringify(safeConfig, null, 2)}`);
...
const encrypt = ({ mimeType }) => {
  ...
  client.encrypt(encryptParamsBuilder.build()).then(ct => {
    ct.pipe(dstAsStream(program.dstFile, "binary"));
    ct.on("end", done);
  });
};
...
const decrypt = () => {
  ...
  client.decrypt(decryptParams).then(pt => {
    pt.pipe(dstAsStream(program.dstFile, "utf8"));
    pt.on("end", done);
  });
};
