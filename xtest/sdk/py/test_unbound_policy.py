...
OIDC_CONFIGURATION_URL = (
    "http://localhost:65432/auth/realms/tdf/.well-known/openid-configuration"
)
...
def update_policy_binding(zip_file_path, new_policy_binding):
    with open(zip_file_path, "r") as manifest:
        manifest_data = json.load(manifest)
        manifest_data["encryptionInformation"]["keyAccess"][0][
            "policyBinding"
        ] = new_policy_binding
...
def main():
    ...
    oidc_creds.set_client_id_and_client_secret(
        client_id="tdf-client", client_secret="123-456"
    )
    ...
    client.add_data_attribute(
        "https://example.com/attr/Classification/value/M", KAS_URL
    )
    ...
    with zipfile.ZipFile(updated_tdf_file, "w") as zip_ref:
        for folder_name, subfolders, filenames in os.walk(tdf_nopolicy_folder):
            for filename in filenames:
                ...
                zip_ref.write(
                    file_path, os.path.relpath(file_path, tdf_nopolicy_folder)
                )
    ...
    try:
        ...
        if os.path.exists(os.path.join(cwd, "sample_policy.txt")):
            logging.error(
                "Decryption was successful, but it was not expected due to a broken policy."
            )
            sys.exit(1)
        else:
