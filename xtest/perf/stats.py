...
class GateResult:
    ...
    #: At least one comparison against something other than a control was
    #: measured. A clean A/A control alone says nothing about the candidate.
    ...
    def nothing_measured(self) -> bool:
        """True if the run produced no comparison beyond its own controls.

        Not the same thing as "no regressions", though the two are identical
        from the outside: both have an empty ``regressions`` list. Two distinct
        ways to get here look the same from outside but need the same
        treatment: a run where every cell was skipped -- no baseline installed,
        an SDK that would not build -- and a run that measured only its A/A
        controls, which assess the harness rather than the candidate. Either
        way the benchmark's question was never answered, which is how one that
        quietly stopped measuring anything could survive for months unnoticed.
