    ...
    *,
    ...
    Args:
        ...
        deadline: absolute ``clock()`` value past which no new round starts.
            Also caps each invocation's timeout to whatever remains, so one
            slow arm cannot run past the deadline before the round-level
            checks get a chance to notice.
    ...
    def one_round(into: dict[str, dict[str, list[float]]]) -> None:
        # Build the round off to the side. If the deadline expires partway
        # through, none of this partial round may enter the paired sample
        # vectors.
        completed = _empty_samples(arm_ids)
        round_rss_floor = 0
        order = list(arms)
        rng.shuffle(order)
        for arm in order:
