...
def test_cross_roundtrip(encrypt_sdk, decrypt_sdk, owner, stage, serial, pt_file):
    ...
    encrypt(encrypt_sdk, owner, stage, pt_file, ct_file, mime_type="text/plain")
...
def encrypt(sdk, owner, stage, pt_file, ct_file, mime_type="application/octet-stream", attributes=""):
    if attributes:
        c = [
            sdk,
            owner,
        ...
        c = [sdk, owner, stage, "encrypt", pt_file, ct_file, "--mimeType", mime_type]
    logger.info("Invoking subprocess: %s", " ".join(c))
    subprocess.check_call(c)


def decrypt(sdk, owner, stage, ct_file, rt_file):
    c = [sdk, owner, stage, "decrypt", ct_file, rt_file]
    logger.info("Invoking subprocess: %s", " ".join(c))
    subprocess.check_call(c)
...
def setup(crud, stage, owner):
    teardown(crud, stage, owner)
    os.makedirs(tmp_dir)


def teardown(crud, stage, owner):
    dirs = [tmp_dir]
