"""The GMAC root downgrade (DSPX-4703).

``rootSignature.alg: GMAC`` means "the trailing 16 bytes of the aggregate
hash" -- the concatenation of the segment hashes, which AES-GCM has never
touched. There is no tag to extract, so the result is a copy of the last
...
HS256-rooted TDF can be downgraded onto that branch.

``segmentHashAlg: GMAC`` is the opposite case and is sound. AES-GCM has just
produced a tag over exactly those bytes under the DEK, so reading it out is a
genuine MAC obtained for free. Every SDK writes an HS256 root and GMAC
segments, unconditionally -- there is no CLI flag for either, which is what
:func:`test_default_integrity_algorithms` pins down.
...
The exploit cases have no flag to gate on and gate on an observed rejection
instead -- see :func:`skip_unless_gmac_root_rejected`.
...
def reverse_payload_segments(sizes: list[int]) -> PayloadChange:
    ...
    def change(payload: bytes) -> bytes:
        return b"".join(reversed(slice_segments(payload, sizes)))
...
def duplicate_payload_segment(sizes: list[int], index: int) -> PayloadChange:
    ...
    def change(payload: bytes) -> bytes:
        chunks = slice_segments(payload, sizes)
        return b"".join(chunks[: index + 1] + [chunks[index]] + chunks[index + 1 :])

    return change


def slice_segments(payload: bytes, sizes: list[int]) -> list[bytes]:
    """Cut ``0.payload`` into its ciphertext segments."""
...
# --- the integrity defaults every SDK writes ---------------------------------


def test_default_integrity_algorithms(
    encrypt_sdk: tdfs.SDK,
    in_focus: set[tdfs.SDK],
    attribute_default_rsa: Attribute,
    encrypted_tdf: EncryptFactory,
) -> None:
    """Every SDK emits an HS256 root and GMAC segments, with no way to ask otherwise.

    No SDK exposes an integrity-algorithm option: the root is HMAC and the
    segments are the AES-GCM tags, hardcoded, and all 8 golden TDFs in
    ``golden/`` carry that pair. Writing it down turns an undocumented
    assumption into a tripwire -- an SDK that started emitting something else
    fails here instead of quietly shifting what every other test measures, and
    a GMAC root in particular is forgeable without any key (DSPX-4703).

    The digest lengths matter as much as the labels: an SDK that says GMAC and
    hashes with HS256 would satisfy the label check alone, and its files would
    still round-trip against itself.
    """
    ...
    for i, segment in enumerate(ii.segments):
        ...
        assert got == tdfs.GMAC_TAG_BYTES, (
            f"{encrypt_sdk} segment {i} hash decodes to {got} bytes; GMAC "
            f"implies {tdfs.GMAC_TAG_BYTES}"
        )
...
def multi_segment_ct(
    ...
    Written with default options -- an HS256 root and GMAC segments, which is
    all any SDK can write -- because the finding is that a *normal* file can be
    downgraded. No SDK feature gate here: producing this file needs nothing
    new, and the attack itself needs no key at all.
    """
...
def skip_unless_gmac_root_rejected(
    ...
    reach. Nor does it change anything a writer emits -- every SDK already
    writes an HS256 root, before and after the fix -- so there is no manifest
    difference to detect either.
