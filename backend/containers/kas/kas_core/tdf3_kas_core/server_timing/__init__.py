...
# duplicated kas/lib/tdf3_kas_core/server_timing eas/src/server_timing
import datetime
import logging
...
class Timing:
    ...
    def __init__(self, app, host, port, service):
        ...
        try:
            ...
            Timing.log.info(f"StatsClient not found {host}:{port}")
            Timing.log.debug(e)
        if app.debug:
            Timing.debug = True
            Timing.log.info(
    ...
    def stop(key):
        ...
        if _key not in request.context:
            Timing.log.warning(f"Key '{key}' not found in request context")
            return
